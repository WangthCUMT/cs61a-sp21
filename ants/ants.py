...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        curr_range = 0
        while curr_range < self.min_range and not place.is_hive:
            place = place.entrance
            curr_range += 1
        while curr_range < self.max_range and not place.is_hive:
            if len(place.bees) > 0:
                break
            place = place.entrance
            curr_range += 1
        if place.is_hive or curr_range > self.max_range:
            return None
...
class FireAnt(Ant):
    ...
    def damage_all_bees(self,place, amount):
        same_place_bees = place.bees[:]
    ...
    def reduce_health(self, amount):
        ...
        self.damage_all_bees(damaging_place, amount)
        if self.place is None:
            self.damage_all_bees(damaging_place, self.damage)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        FireAnt.damage_all_bees(self, self.place, self.damage)
        ...
        while p is not None:
            if isinstance(p.ant, ContainerAnt):
                if p.ant.ant_contained is not None:
                    ants.append(p.ant)
                    ants.append(p.ant.ant_contained)
                else:
                    ants.append(p.ant)
            ants.append(p.ant)
            p = p.exit
