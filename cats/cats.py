...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def select(p):
        words = lower(remove_punctuation(p)).split(' ')
        for w in words:
...
def accuracy(typed, source):
    ...
    if typed == '' and source == '':
        ...
        correct = sum([1 for x, y in zip(typed_words, source_words) if x == y])
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    l_diff = [diff_function(typed_word,i,limit) for i in word_list]
    if typed_word in word_list:
        return typed_word
    elif min(l_diff,key=abs)>limit:
        return typed_word
    else:
        return word_list[l_diff.index(min(l_diff,key=abs))]
...
def report_progress(typed, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    progress = sum([1 for x , y in zip(typed, prompt) if x == y])/ len(prompt)
    d = {}
    d['id'] = user_id
    d['progress'] = progress
    upload(d)
...
def fastest_words(match):
    ...
    for i in player_indices:
        ...
        time_lists[list_name] = Times[i]
    player_list1 = time_lists['list_0']
    player_list2 = time_lists['list_1']
    w_check1 = []
    w_check2 = []
    for j in word_indices:
        w_check1.append(player_list1[j] <= player_list2[j])
        w_check2.append(player_list1[j] > player_list2[j])
    word_list1 = []
    word_list2 = []
    for j in word_indices:
        if w_check1[j]:
            word_list1.append(Words[j])
    for j in word_indices:
        if w_check2[j]:
            word_list2.append(Words[j])
    l_final = []
    l_final.append(word_list1)
    l_final.append(word_list2)
    return l_final
