...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        def eval_one_argument(expr):
            return scheme_eval(expr,env=env)
        procedure = scheme_eval(first, env=env)
        rest_list = rest.map(eval_one_argument)
        return scheme_apply(procedure,rest_list,env=env)
        # END PROBLEM 3
...
def pair_to_list(s):
    if s is nil:
        ...
        return [s.first] + pair_to_list(s.rest)